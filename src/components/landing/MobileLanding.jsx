...
export default function MobileLanding() {
  ...
  useEffect(() => {
    let datas = [];
    if (categories) {
      datas = categories?.filter((cat) => cat?.display_in_home);
    }
    setProductCategories(datas);
  }, [categories]);
  ...
  return (
    <>
      <div className="w-full flex flex-col justify-start items-center mx-auto">
        ...
        {/* home categories */}
        <div className="w-full flex justify-between items-start flex-wrap gap-2 px-5">
          {productCategories && productCategories?.length !== 0
            ? productCategories?.map((cat) => (
                <Link
                  href={`/filters?slug=${cat?.slug}`}
                  key={cat?.id}
                  className="w-[48%]"
                >
                  <Image
                    src={imageUrl + cat?.image_landing}
                    alt={cat.name}
                    // onError={(e) =>
                    //   setProductCategories(
                    //     productCategories.filter((ct) => ct?.id !== cat?.id)
                    //   )
                    // }
                    width={100}
                    height={100}
